    ...
    *,
    ...
    for idx, item in enumerate(raw_items):
        ...
        if item_type == "function_call":
            ...
            if not _RESPONSES_FUNCTION_NAME_PATTERN.fullmatch(name.strip()):
                # Historical tool-call records can survive a tool rename or an
                # older provider's looser naming rules. The Responses endpoint
                # rejects the entire request for one invalid historic name, so
                # drop this obsolete call and its paired output below.
                dropped_call_ids.add(call_id.strip())
                logger.warning(
                    "Dropping historical Responses function_call with invalid name %r",
                    name[:120],
                )
                continue
        ...
        if item_type == "function_call_output":
            ...
            if call_id.strip() in dropped_call_ids:
                continue
