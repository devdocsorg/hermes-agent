...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    payload and XML comments when deciding whether the installed plist is
    stale. Comments are operator documentation, not launchd behavior; treating
    a wording-only edit as stale can cause an unnecessary service reload.
    """
    import re
...
def generate_launchd_plist() -> str:
    ...
    # Keep launchd's SIGKILL deadline beyond both the configured graceful
    # drain and post-drain cleanup. Unlike systemd, launchd manages the
    # gateway's whole process tree, so a 25-second fixed deadline can discard
    # queued follow-ups during a legitimate restart. Preserve a 150-second
    # minimum for the default and short drain configurations.
    drain_timeout = int(_get_restart_drain_timeout() or 0)
    exit_timeout = max(150, drain_timeout + 30)
...
<dict>
    ...
    <!-- ThrottleInterval raises launchd's default 10s minimum respawn interval
         ...
         respawn storm; ExitTimeOut leaves room for the configured graceful
         drain plus cleanup before launchd escalates from SIGTERM to SIGKILL. -->
