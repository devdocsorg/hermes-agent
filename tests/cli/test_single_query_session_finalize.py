...
def test_human_single_query_main_preserves_failed_turn_exit_code(monkeypatch):
    calls = []
    ...
    class FakeCLI:
        def __init__(self, **_kwargs):
            self.console = _Console()
            self.session_id = "failed-single-query-session"
            self.agent = SimpleNamespace(
                session_id="failed-single-query-session",
                platform="cli",
            )
    ...
    assert calls == [
        ...
        ("chat", "hello", None),
        "summary",
        ("finalize", "failed-single-query-session"),
    ]
