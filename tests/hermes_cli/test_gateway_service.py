...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_adds_cleanup_headroom_to_positive_drain_timeout(
        self, monkeypatch
    ):
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(
        self, tmp_path, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            gateway_cli.shutil,
            "which",
            lambda cmd: str(link_node) if cmd == "node" else None,
        )
    ...
    def test_launchd_plist_does_not_leak_profile_node_symlink_target(
        self, tmp_path, monkeypatch
    ):
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_plist_preserves_graceful_drain_before_sigkill(
        self,
        monkeypatch,
        drain_timeout,
        expected_exit_timeout,
    ):
        monkeypatch.setattr(
            gateway_cli,
            "_get_restart_drain_timeout",
        ...
        assert (
            f"<key>ExitTimeOut</key>\n    <integer>{expected_exit_timeout}</integer>"
            in plist
        )

    def test_launchd_staleness_comparison_ignores_xml_comments(self):
        installed = """
<plist>
<!-- Existing operator note. -->
<dict><key>ExitTimeOut</key><integer>150</integer></dict>
</plist>
"""
        generated = """
<plist>
<!-- Updated documentation that does not change launchd behavior. -->
<dict><key>ExitTimeOut</key><integer>150</integer></dict>
</plist>
"""
        ...
        assert gateway_cli._normalize_launchd_plist_for_comparison(
            installed
        ) == gateway_cli._normalize_launchd_plist_for_comparison(generated)
...
class TestSystemUnitRefreshSyncsHermesHome:
    ...
    def test_refresh_adopts_unit_hermes_home_before_rewriting(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway_cli, "_sync_hermes_home_from_systemd_unit", tracking_sync
        )
        monkeypatch.setattr(Path, "read_text", tracking_read_text)
        # Avoid a real generate/compare — we only assert sync precedes read.
        monkeypatch.setattr(
            gateway_cli, "generate_systemd_unit", lambda **k: "[Unit]\n"
        )
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            calls = []
            monkeypatch.setattr(
                gateway_cli, "_select_systemd_scope", lambda system=False: True
            )
            monkeypatch.setattr(
            ...
            assert "refresh" in calls, (
                f"{entry} must call refresh_systemd_unit_if_needed"
            )
    ...
    def test_timeout_expired_is_retried_not_escaped(self, monkeypatch):
        ...
        def fake_run(cmd, check=False, **kwargs):
            ...
            if cmd[:2] == ["launchctl", "list"]:
                ...
                return SimpleNamespace(
                    returncode=0 if attempts["bootstrap"] >= 2 else 1
                )
            return SimpleNamespace(returncode=0, stdout="", stderr="")
