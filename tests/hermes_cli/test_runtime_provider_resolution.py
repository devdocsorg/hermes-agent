...
def test_loopback_custom_codex_proxy_stays_on_responses_runtime(monkeypatch):
    model_cfg = {
        "provider": "custom:codex-auto",
        "default": "codex-auto",
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: {
            ...
            "source": "custom_provider:Codex Auto LiteLLM",
        },
    )

    resolved = rp.resolve_runtime_provider(requested="custom:codex-auto")

    assert resolved["api_mode"] == "codex_responses"


@pytest.mark.parametrize(
    "requested,api_mode,base_url",
    [
        ("custom:other", "codex_responses", "http://127.0.0.1:4400/v1"),
        ("custom:codex-auto", "chat_completions", "http://127.0.0.1:4400/v1"),
        ("custom:codex-auto", "codex_responses", "https://proxy.example.com/v1"),
    ],
)
def test_custom_provider_app_server_gate_fails_closed(
    monkeypatch,
    requested,
    api_mode,
    base_url,
    ...
    assert resolved["api_mode"] == api_mode
...
def test_resolve_named_custom_runtime_pool_result_includes_extra_headers(monkeypatch):
    ...
    monkeypatch.setattr(
        rp, "_try_resolve_from_custom_pool", lambda *a, **k: pool_return_value
    )
    monkeypatch.setattr(
        rp,
        "_get_named_custom_provider",
